...
function AddFoodModal({ onAdd, onClose }) {
  ...
  const filtered   = Object.keys(FOOD_DB).filter(f => f.toLowerCase().includes(search.toLowerCase()));
  const food       = selected ? FOOD_DB[selected] : null;
  const multiplier = food ? (qty * food.gramsPerUnit) / 100 : 0;
  ...
  function handleAdd() {
    if (!selected || qty <= 0) return;
    onAdd({ name: selected, qty, unit: food.unit, ...preview });
    onClose();
  }
...
export default function TrackerDashboard({
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: C.bg, minHeight: "100vh", color: C.text }}>
      ...
      {modal && (
        <AddFoodModal onAdd={(item) => handleAdd(modal, item)} onClose={() => setModal(null)} />
      )}
    </div>
  );
}
