...
function HealthCalculator({ onStartTracking }) {
  ...
  // Called when user clicks "Start Tracking Calories"
  function handleStartTracking() {
    const weightKg   = parseFloat(weight) || 70;
    const finalCal   = useCustomCal && customCal
      ? parseInt(customCal)
      : getGoalCalories()[selectedGoal];
      ...
      {showResults && calories && (
        <div>
          ...
          <div className="goalsGrid">
            {[
              ...
              <div
                ...
                className="goalCard"
                style={{
                  borderColor: selectedGoal === g.key ? g.color : "rgba(255,255,255,0.1)",
                  background:  selectedGoal === g.key ? `${g.color}18`  : "rgba(255,255,255,0.03)",
                }}
          ...
          {(() => {
            ...
            const m = getMacros(finalCal, selectedGoal, weightKg);
            return (
              <div style={{
                ...
                {[
                  ...
                  <div key={macro.label} style={{
                    ...
                    <div style={{ fontSize: "20px", fontWeight: 600, color: macro.color, fontFamily: "'DM Mono', monospace" }}>
                      {macro.val}g
                    </div>
                    <div style={{ fontSize: "11px", color: "#6a6a62", marginTop: "3px", textTransform: "uppercase", letterSpacing: "0.5px" }}>
                      {macro.label}
                    </div>
                  </div>
                ))}
          ...
          {/* ACTION BUTTONS */}
          <div className="actionButtons">
            <button className="actionButton" onClick={handleStartTracking}>
              Start Tracking →
            </button>
          </div>
        </div>
...
function IndiFitLanding({ onStartTracking }) {
  return (
    <div className="page">
      ...
      {/* Pass the onStartTracking callback into HealthCalculator */}
      ...
      <section className="section" style={{ paddingBottom: "3rem" }}>
        ...
        <div className="chipGrid">
          {FOODS.map((food, i) => (
            <div key={i} className="chip">
              {food.name}
              {food.protein && <span className="chipMacro">{food.protein}</span>}
            </div>
          ))}
        </div>
      </section>
...
export default function App() {
  ...
  // This is the data that flows from the health calculator → tracker
  const [userData, setUserData] = useState(null);

  function handleStartTracking(data) {
    setUserData(data);   // save the user's details
    setPage("tracker");  // switch page
  }
  ...
  if (page === "tracker" && userData) {
    return (
      <TrackerDashboard
        ...
        onBack={() => setPage("landing")}  // optional back button
      />
    );
  }
